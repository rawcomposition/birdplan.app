...
Map marker PNGs in `frontend/public/markers/` are generated by `npm run generate-markers` (`frontend/scripts/generate-markers.ts`, needs a global `sharp` symlinked into `node_modules`); re-run after changing `markerIcons` or `markerColors`.
