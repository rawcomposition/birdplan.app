    ...
    <div className="mt-4 rounded-lg border border-border/70 bg-muted/40 px-3 py-2.5 print:break-inside-avoid">
      <div className="flex items-center gap-1.5">
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <span aria-label="What this means" className="cursor-default text-[11px] text-muted-foreground/70 print:hidden">
            ...
            <ul className="mt-0.5 flex flex-col gap-0.5">
              {group.targets.map((it) => (
                <li key={it.code} className="flex items-baseline gap-2 text-[13px]">
                  ...
                  <span className="ml-auto shrink-0 tabular-nums text-muted-foreground">{formatFrequency(it.frequency)}%</span>
                </li>
              ))}
            </ul>
          </div>
