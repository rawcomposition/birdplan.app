...
export default function Explore() {
  ...
  const hotspotClick = (id: string) => {
    const saved = savedHotspots.find((it) => it.hotspotId === id);
    ...
    <div className="flex flex-col h-full">
      ...
      <main className="flex flex-1 min-h-0 relative bg-background">
        <ErrorBoundary>
          ...
          {notice && (
            <Card className="absolute top-3 left-1/2 z-10 -translate-x-1/2 px-4 py-2 text-sm text-muted-foreground shadow-md whitespace-nowrap">
              {notice}
            </Card>
          )}
